...
class ResponseManager {
    constructor() { }
    #generateResponseFormat(message: string, statusCode: number, data: any): ResponseFormat {
        return {
            statusCode,
            message,
            data
    ...
    // 4xx Client Errors
    badRequest(res: Response, message: string, data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 400, data);
        res.status(responseData.statusCode).json(responseData);
    }

    unauthorized(res: Response, message: string = 'Unauthorized', data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 401, data);
        res.status(responseData.statusCode).json(responseData);
    }

    forbidden(res: Response, message: string = 'Forbidden', data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 403, data);
        res.status(responseData.statusCode).json(responseData);
    }

    notFound(res: Response, message: string = 'Not Found', data: any = {}): void {
    ...
    unprocessableEntity(res: Response, message: string, data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 422, data);
        res.status(responseData.statusCode).json(responseData);
    }

    // 5xx Server Errors
    error(res: Response, message: string = 'Internal server error', data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 500, data);
        res.status(responseData.statusCode).json(responseData);
    }

    notImplemented(res: Response, message: string = 'Not Implemented', data: any = {}): void {
        const responseData = this.#generateResponseFormat(message, 501, data);
        res.status(responseData.statusCode).json(responseData);
    }

    serviceUnavailable(res: Response, message: string = 'Service Unavailable', data: any = {}): void {
